/// <reference path="../../types/wusaki-dashboard.d.ts" />
...
function dmemRenderDeleteBtn(_value: unknown, item: Record<string, unknown>): string {
  ...
  return `<button class="icon-btn row-delete-btn" type="button" onclick="event.stopPropagation();void(async()=>{try{await api('/api/dashboard/memories/batch-delete',{method:'POST',body:JSON.stringify({ids:['${id}']})});window.dispatchEvent(new CustomEvent('wusaki-dashboard-refresh'))}catch(e){alert(e.message||String(e))}})()" title="删除此条">✕</button>`;
}
...
window.WusakiDashboard.registerPlugin({
  id: "default_memory",
  label: "Memory",
  viewLabel: "memory",
  pageSize: 25,
