"""
proactive_v1/tools.py — Tool schemas + execute dispatcher
...
from proactive_v1.outbound_text import normalize_outbound_text
...
def _get_content(ctx: AgentTickContext, args: dict) -> str:
    ...
    if unknown:
        logger.warning("[proactive_v1] get_content: unknown item_ids=%s (valid=%s)", unknown, sorted(valid))
        return json.dumps(
...
def _mark_interesting(ctx: AgentTickContext, args: dict) -> str:
    ...
    if unknown and valid:
        logger.warning("[proactive_v1] mark_interesting: unknown item_ids=%s", unknown)
        return json.dumps(
    ...
    logger.info(
        "[proactive_v1] classified interesting ids=%s reason=%s",
        item_ids,
        reason or "(none)",
    )
...
def _mark_not_interesting(ctx: AgentTickContext, args: dict) -> str:
    ...
    unknown = [i for i in item_ids if i not in valid]
    if unknown and valid:
        logger.warning("[proactive_v1] mark_not_interesting: unknown item_ids=%s", unknown)
        return json.dumps(
            {"error": f"以下 id 不在本轮候选列表中，无法标记：{unknown}。本轮有效 id：{sorted(valid)}"},
            ensure_ascii=False,
        )
    ...
    logger.info(
        "[proactive_v1] classified not_interesting ids=%s reason=%s",
        item_ids,
        reason or "(none)",
    )
    return json.dumps({"ok": True}, ensure_ascii=False)
