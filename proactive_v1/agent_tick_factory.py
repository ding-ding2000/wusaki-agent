...
from proactive_v1.drift_state import DriftStateStore
from proactive_v1.drift_tools import DriftToolDeps
from proactive_v1.gateway import GatewayDeps
from proactive_v1.tools import ToolDeps
...
class AgentTickFactory:
    ...
    def build(self) -> ProactiveTurnPipeline:
        if self._deps.pool is None:
            raise RuntimeError("proactive_v1 依赖 MCP 连接池，pool 不能为空")
    ...
    def _build_llm_fn(self) -> LlmFn:
        ...
        async def llm_fn(
            ...
            if not resp.tool_calls:
                ...
                logger.warning(
                    "[proactive_v1] llm_fn: no tool call returned (text=%r)",
    ...
    def _build_tool_deps(self) -> ToolDeps:
        ...
        try:
            ...
            logger.warning("[proactive_v1] web_fetch 不可用，已降级禁用: %s", e)
        return ToolDeps(
