...
from proactive_v1.fitbit_sleep import (
    FitbitSleepProvider,
    SleepContext,
...
async def test_fitbit_sleep_provider_and_bootstrap_paths(
    ...
    monkeypatch.setattr("proactive_v1.fitbit_sleep.time.sleep", _sleep)
    with pytest.raises(RuntimeError, match="stop loop"):
        provider2._loop()
    assert sleep_calls[0] == 5

    monkeypatch.setattr("proactive_v1.fitbit_sleep.asyncio.to_thread", AsyncMock(return_value=None))
    await _bootstrap_monitor_data("http://x", max_wait_sec=1)

    async def _fail_to_thread(*args, **kwargs):
        raise RuntimeError("not ready")

    monkeypatch.setattr("proactive_v1.fitbit_sleep.asyncio.to_thread", _fail_to_thread)
    monkeypatch.setattr("proactive_v1.fitbit_sleep.asyncio.sleep", AsyncMock(return_value=None))
    monotonic_values = [0.0, 0.2, 1.2]
    monkeypatch.setattr(
        "proactive_v1.fitbit_sleep.time.monotonic",
...
async def test_run_fitbit_monitor_restart_loop(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    monkeypatch.setattr("proactive_v1.fitbit_sleep.asyncio.create_subprocess_exec", _create_proc)
    monkeypatch.setattr("proactive_v1.fitbit_sleep._bootstrap_monitor_data", _never_done)
    monkeypatch.setattr("proactive_v1.fitbit_sleep.asyncio.sleep", AsyncMock(return_value=None))
