...
class Memorizer:
        ...
        if history_entry and history_entry.strip():
            try:
                ...
                if self._store.has_consolidation_source_ref(source_ref):
                    logger.info(
                        "memory consolidation skip duplicated source_ref=%s",
                        source_ref,
                    )
                    text = ""
                if text:
                ...
                if text:
                    ...
                    if result.startswith("skipped:"):
                        ...
                        logger.info(f"memory event saved: {result}")
            except Exception as e:
                logger.warning(f"memory event save 失败: {e}")

        # 2. behavior_updates 统一由 post-response worker 处理，避免与 consolidation 重复写入
        if behavior_updates:
            logger.info(
                "memory consolidation skip behavior_updates (%d): handled by post-response worker",
    ...
    def _should_semantic_dedup_event(
        ...
        *,
        ...
        logger.info(
            "memory event semantic-dedup: similar=%s",
            similar_ids[:1],
        )
        return True
    ...
    def supersede_batch(self, ids: list[str]) -> None:
        ...
        logger.info(f"memory superseded {len(ids)} items: {ids}")
