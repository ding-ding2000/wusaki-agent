...
class Retriever:
    ...
    async def retrieve(
        ...
        logger.debug(
            "memory retrieve: query=%r vector=%d keyword=%d fused=%d",
            query[:60],
            len(vector_items),
            len(keyword_items),
            len(items),
    ...
    async def _retrieve_vector_lanes(
        ...
        *,
        ...
        try:
            ...
            logger.debug("memory retrieve: vector_search_batch failed: %s", e)
        ...
        for vector in vectors:
            try:
                ...
                logger.debug("memory retrieve: vector_search failed: %s", e)
                continue
            for hit in hits:
                _remember_vector_hit(seen, hit)
        return list(seen.values())
    ...
    async def _embed_lanes(self, query_texts: list[str]) -> list[list[float]]:
        ...
        for result in results:
            if isinstance(result, BaseException):
                logger.warning("memory retrieve: embed failed, fallback lane skipped: %s", result)
                continue
    ...
    async def retrieve_with_vec(
        ...
        logger.debug(f"memory retrieve_with_vec: hits={len(items)}")
        return items
