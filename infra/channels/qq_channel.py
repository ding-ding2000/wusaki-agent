...
_NCATBOT_DIR = Path.home() / ".wusaki" / "ncatbot"
...
def _patch_ncatbot_ws_open_timeout(timeout_seconds: float) -> None:
    ...
    try:
        ...
        original_connect = getattr(
            adapter_mod,
            "_wusaki_original_websockets_connect",
            None,
        )
        if original_connect is None:
            original_connect = adapter_mod.websockets.connect
            adapter_mod._wusaki_original_websockets_connect = original_connect

            def _patched_connect(*args, **kwargs):
                configured_timeout = getattr(
                    adapter_mod,
                    "_wusaki_websocket_open_timeout_seconds",
                    None,
                )
                if configured_timeout is not None:
                    kwargs["open_timeout"] = configured_timeout
                return adapter_mod._wusaki_original_websockets_connect(*args, **kwargs)

            adapter_mod.websockets.connect = _patched_connect

        adapter_mod._wusaki_websocket_open_timeout_seconds = timeout_seconds
...
class QQChannel:
    ...
    def __init__(
        ...
        # Wusaki 只需要 NapCat 的 OneBot WebSocket，禁用 WebUI 避免启动时卡交互 token。
        ncatbot_config.napcat.enable_webui = False
        ncatbot_config.enable_webui_interaction = False
        # 运行时产物重定向到 ~/.wusaki/ncatbot/，不污染项目目录
