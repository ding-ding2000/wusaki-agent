...
class IPCServerChannel:
    def __init__(
        ...
        self._bound_endpoint = self._socket_path
    ...
    async def start(self) -> str:
        ...
        if not hasattr(asyncio, "start_unix_server"):
            return await self._start_fallback_tcp()
        ...
        try:
            ...
            logger.warning(
                "IPC unix socket bind failed for %s (%s); falling back to tcp",
                self._socket_path,
                exc,
            )
            return await self._start_fallback_tcp()
    ...
    async def stop(self) -> None:
        ...
        if _parse_tcp_endpoint(self._bound_endpoint) is None:
            Path(self._bound_endpoint).unlink(missing_ok=True)

    async def _start_fallback_tcp(self) -> str:
        try:
            self._server = await asyncio.start_server(
                self._handle_connection,
                host="127.0.0.1",
            ...
            self._disabled = True
            self._bound_endpoint = "disabled"
            logger.warning(
                "IPC fallback TCP bind failed (%s); IPC channel disabled for this run",
                exc,
            )
            return self._bound_endpoint
        sock = self._server.sockets[0] if self._server.sockets else None
        if sock is None:
            raise RuntimeError("Failed to determine fallback TCP endpoint.")
        host, port = sock.getsockname()[:2]
        self._bound_endpoint = f"{host}:{port}"
        logger.info("IPC server listening on tcp://%s:%s", host, port)
        return self._bound_endpoint
