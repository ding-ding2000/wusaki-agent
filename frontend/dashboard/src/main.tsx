...
function App(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    window.addEventListener("wusaki-dashboard-refresh", refresh);
    return () => window.removeEventListener("wusaki-dashboard-refresh", refresh);
    ...
    <div className="shell">
      <header className="topbar">
        <div className="brand">
          ...
          <div>
            <div className="brand-title">Wusaki Dashboard</div>
...
function formatPluginCell(plugin: PluginConfig, column: DashboardColumn, item: Record<string, unknown>): string {
  ...
  const formatter = plugin.formatters?.[column.fmt || ""] ?? (window as Window & { WusakiDashboard?: { _formatters: Record<string, (value: unknown, item?: Record<string, unknown>) => string> } }).WusakiDashboard?._formatters[column.fmt || "text"];
